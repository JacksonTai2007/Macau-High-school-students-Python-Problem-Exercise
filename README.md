...
- **`Python-Competition-Practise-v2.html`（最新版，推薦）** — 全面重寫版本：
  - 現代化介面、深 / 淺色主題切換、作答進度條與計時，**完整手機端適配**（直向佈局、加大觸控目標、安全區域）。
  - **題庫瀏覽 / 關鍵字搜尋**：跨題目、選項、知識點即時搜尋並標示關鍵字。
  - **每日打卡與成就徽章**：連續 / 本月 / 累計打卡天數、最近 14 天打卡圖、8 種成就徽章（作答完成自動打卡）。
  - **按賽段（初賽 / 決賽）與屆數篩選**題目。
  - 多種練習模式：全部題目、只練選擇題、只練程式 / 簡答題、**錯題重練**、**我的收藏**。
  - **背題模式**：閃卡式瀏覽題庫，逐題或一鍵展開答案、知識點與解題思路，純複習不計分。
  - **題目導航面板**：以格狀總覽各題已答 / 未答 / 已標記狀態，可一鍵跳題。
  - **標記待複查**：作答時可為題目插旗，交卷前提示。
  - **成績匯出 / 分享**：一鍵分享、複製或下載成績單（含錯題清單）。
  - **列印 / 存為 PDF**：透過瀏覽器列印功能輸出成績與逐題解析。
  - **鍵盤快捷鍵**：作答時用數字 / 字母選答、`J`/`K` 跳題、`F` 標記、`M` 開合導航、`Ctrl+Enter` 交卷、`?` 查看說明。
  - **PWA 可安裝**：支援「加到主畫面」，載入後即可離線使用（純單檔、無需網路）。
  - **無障礙**：鍵盤聚焦樣式、`aria-label` 標註與彈窗 `role`。
  - 可選作答時限（到時自動交卷）、題目隨機或依原順序。
  - 自動以瀏覽器 `localStorage` 暫存作答進度（可續答）、記錄錯題本與收藏。
  - **學習報告**（首頁）：作答次數、平均 / 最佳得分、累計時間、**連續作答天數**與最近成績趨勢圖、歷史記錄清單。
  - 交卷後提供 **知識點掌握分析**（各知識點正確率，由弱到強排序）與逐題解析（對 / 錯標示、正確答案、知識點、解題思路）。
