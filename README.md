...
- **`Python-Competition-Practise-v2.html`（最新版，推薦）** — 全面重寫版本：
  ...
  - **即時對答案模式**：可選「每題即時對答案」，選完立即揭曉對錯並顯示解析；程式 / 簡答題可隨時查看參考答案。
  - **知識點專練**：依知識點篩選題目，針對弱項集中練習。
  - **自訂抽題數量**：隨機抽 10 / 20 / 30 題做小卷快測。
  - **錯題筆記**：在解析頁與背題模式為每題寫筆記，自動儲存於本機。
  - **資料備份 / 還原**：一鍵匯出 / 匯入所有本機記錄（錯題本、收藏、筆記、歷史、打卡），方便換裝置。
  - **字體大小調整**：標準 / 大 / 特大三檔切換並記憶偏好。
